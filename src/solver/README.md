...
The solver groups its state by responsibility, rather than keeping inlet flags among integration parameters:

| Internal type | Responsibility |
| --- | --- |
| `SPHProperties` | Uniform physical inputs and derived constants; constructs current stage parameters without caching a second physical configuration. |
| `SPHTimeInterval` | Stability limit, quantized duration and integer interval; reused for acoustic and advection steps. |
| `SPHStepState` | The two time intervals, reduced fluid statistics, represented time, pending DEM steps, advection/search phase and coupling interval. |
| `SPHJetState` | Jet particle ranges, final end time and the represented-time completion fast path. |
| `SPHInitializationState` | Initialized population counts and static-boundary classification. |
| `outputSnapshot` | Fluid/boundary state, integration state, coupling history and jet completion for reversible observation. |

CPU and GPU collect statistics independently but share acoustic-limit checking, time quantization, search-buffer calculation and advection-phase advancement. CPU interaction inputs are constructed in one place. Both jet overloads share cylindrical particle generation, preserving their original order and opposite upstream/downstream placement.
...
CPU, GPU, and Hybrid execution apply an active jet at the acoustic-substep start before optional advection preparation and again immediately after velocity integration. Particles that remain inside the virtual pipe carry an internal `constrained` marker, retain their current density and pressure during density reinitialization, and skip continuity-equation density integration. The marker clears as soon as a particle leaves the pipe or the source ends. A constraint active at the start remains active for the complete substep, so its end time is rounded upward by at most one acoustic substep. After the last jet ends, `SPHJets_.completed_` skips all remaining range and pipe checks. This internal flag follows represented fluid time; the public `SPHJetsCompleted()` checks the solver's visible time. A temporary output flush can cross the final end, so snapshots restore the saved completion flag as well as particle constraints. Immutable jet definitions/end times are not copied every output frame.
